...
class Uploadclientmappingfoliokarvy extends Component {
  constructor(props) {
    super(props);
  ...
  addnewclient(e) {
    ...
    axios.post('http://localhost:3001/api/addfoliokarvy', { name: name, pan: pan, data: newdataarray })
      .then(res => {
    ...
    if (checkRadio != undefined) {
      if (checkbelowRadio != undefined) {
        ...
        if (checkRadiovalue === pan) {
          ...
          axios.post('http://localhost:3001/api/savefoliokarvytest', { userid: userid, pan: pan, name: name,add1:add1,add2:add2,add3:add3, data: newarray })
            .then(res => {
              console.log("res=", res)
              if (res.data.status === 200) {
                ...
                var dataarray = [];
                this.state.data1.map((val, key) => {
                  console.log("pan=", val.PANGNO, "name=", val.INVNAME, "below pan", pan)
                  if (val.PANGNO != pan) {
                    dataarray.push(val);
                    console.log("dataarray if=", dataarray)
                    $("#datasection").css("display", "block");
                    ...
                    if(checkRadioarrayvalue === val.INVNAME){
                    ...
                    <table id="example1" className="table table-bordered table-striped">
                      ...
                      <tbody id="qw">
                        {this.state.data1.map((item, index) => (
                          <tr>
                            <td>
                              <input type="radio" name="check" id={item.INVNAME} class={index} value={item.PANGNO} onClick={this.getPan.bind(item.PANGNO)} /></td>
                            <td class="client-name">{item.INVNAME}</td>
                            <td>{item.PANGNO}</td>
                            <td>{item.ACNO}</td>
                            <td>{item.ADD1+item.ADD2+item.ADD3}</td>
                            <td>{item.GUARDPANNO}</td>
                            <td><a href="#" className="btn btn-sm btn-primary" onClick={() => this.addnewclient(item.INVNAME + "/" + item.PANGNO + "/" + index)}>NEW CLIENT</a></td>
                          </tr>
                        ))}
...
export default Uploadclientmappingfoliokarvy;
