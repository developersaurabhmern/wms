...
class Uploadclientmappingtranscams2A extends Component {
  constructor(props) {
    super(props);
  ...
  mergeClient = (e) => {
    ...
    if(checkRadio != undefined) {
      ...
      axios.post('http://localhost:3001/api/savecams2atest', { userid:userid,pan:pan,name:name,data: newarray })
      .then(res => {
        console.log("res=",res)
                  ...
                  <table id="example1" className="table table-bordered table-striped">
                    ...
                    <tbody id="qw">
                      {this.state.data1.map((item, index) => (
                        <tr>
                          <td>
                          <input type="radio" name="check" id={item.INV_NAME} class={index} value={item.PAN1} onClick={this.getPan.bind(item.PAN)} /></td>
...
export default Uploadclientmappingtranscams2A;
