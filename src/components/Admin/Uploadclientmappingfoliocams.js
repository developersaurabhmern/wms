...
class Uploadclientmappingfoliocams extends Component {
  constructor(props) {
    super(props);
  ...
  addnewclient(e) {
    ...
    var add1 = inputvalue[3];
    var add2 = inputvalue[4];
    var add3 = inputvalue[5];
    // alert(name)
    // alert(pan)
    ...
    axios.post('http://localhost:3001/api/addfoliocams', { name: name, pan: pan,add1:add1,add2:add2,add3:add3, data: newdataarray })
      .then(res => {
      ...
      //  console.log("oldarray=", newarray)
        if (checkRadiovalue === pan) {
         // console.log("thisstatedata1", this.state.data1[checkRadio])
          axios.post('http://localhost:3001/api/savefoliocamstest', { userid: userid, pan: pan, name: name,add1:add1,add2:add2,add3:add3, data: newarray })
            .then(res => {
              console.log("res=", res)
              if (res.data.status === 200) {
                ...
                var dataarray = [];
                this.state.data1.map((val, key) => {
                //  console.log("pan=", val.PAN_NO[0], "name=", val.INV_NAME, "below pan", pan)
                  if (val.PAN_NO != pan) {
                ...
                //    console.log("dataarray if=", dataarray)
                    ...
                    if(checkRadioarrayvalue === val.INV_NAME){

                    }else{
                     // console.log("newdataarray=",val)
                      dataarray.push(val);
                      $("#datasection").css("display", "block");
                      $("#uploaddata").css("display", "block");
                      $("#uploaddatamsg").css("display", "none");
                    ...
                    <table id="example1" className="table table-bordered table-striped">
                      ...
                      <tbody id="qw">
                        {this.state.data1.map((item, index) => (
                          <tr>
                            <td>
                              <input type="radio" name="check" id={item.INV_NAME} class={index} value={item.PAN_NO} onClick={this.getPan.bind(item.PAN_NO)} /></td>
                            <td class="client-name">{item.INV_NAME}</td>
                            <td>{item.PAN_NO}</td>
                            <td>{item.FOLIOCHK}</td>
                            <td>{item.ADDRESS1+item.ADDRESS2+item.ADDRESS3}</td>
                            <td>{item.GUARD_PAN}</td>
                            <td><a href="#" className="btn btn-sm btn-primary" onClick={() => this.addnewclient(item.INV_NAME + "#" + item.PAN_NO + "#" + index + "#" + item.ADDRESS1 + "#" + item.ADDRESS2 + "#" + item.ADDRESS3)}>NEW CLIENT</a></td>
                          </tr>
                        ))}
                    ...
                    <div>
                      ...
                      <table id="example2" className="table table-bordered table-striped">
                        ...
                        {this.state.searchdata.map((item, newindex) => (
                          <tbody>
                            <tr>
                              <td><input type="text" id="clientdata" value={item.PAN + "/" + item.INVNAME + "/" + item.USER_ID} hidden /><input type="radio" name="check2" class={newindex} id="check2" value={item.PAN + "#" + item.INVNAME + "#" + item.USER_ID + "#" +item.ADD1 + "#" + item.ADD2 + "#" + item.ADD3} /></td>
...
export default Uploadclientmappingfoliocams;
